 ...
 * 用法：
 *   // 构建主题
 *   String cmdTopic = topic::cmd("0001");        // "vex/0001/cmd"
 *   String statusTopic = topic::status("0101");  // "vex/0101/status"
 *   String eventTopic = topic::event("0201");    // "vex/0201/event"
 *   String cloudCmd = topic::cloudCmd("0001");   // "cloud/vex/0001/cmd"
 *
 *   // 使用消息类型常量（配合 Message 类）
 *   Message msg(TYPE_ENV_STATUS);   // "env.status"
 *   msg.set("temp", 25.3f);
 *
 *   // 在回调中匹配指令类型
 *   if (strcmp(msg.type(), TYPE_DOOR_UNLOCK) == 0) { ... }
 *
 *   // UNIT_ID 由 platformio.ini 编译时注入，如 -D UNIT_ID=\"0001\"
 */
